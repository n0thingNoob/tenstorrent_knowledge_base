...
1. **Tensix Scalar Unit (ThCon)** — atomics on **L1 / L0** within the local Tensix.
2. **NoC** — atomic transactions on **128 b in a remote tile's L1**, returning 32 b to the initiator.
...
Detailed semantics live in `WormholeB0/NoC/Atomics.md` and `WormholeB0/TensixTile/L1.md#atomics` — **not yet in `raw/`**.

## Use cases

- **Software counters across cores / tiles** — increment with NoC atomic; threshold trigger via [[Semaphores]] + STALLWAIT.
- **Lock-free producer/consumer queues** — `ATINCGETPTR` for ring-buffer pointers; underpins potential implementations of [[Circular Buffers]].
- **Distributed barriers / collectives** — atomic decrements signalling "N-1 cores done".
- **Replacing 2-semaphore handshakes** — single counter increment on producer side; consumer reads; less round-tripping than post+wait.

## Research signal

The user's stated interest is in *replacing two-semaphore handshakes with atomic counters*. The hardware primitives above are precisely the surface that enables that. Open question: which TT-Metal/TT-NN ops still use 2-step handshakes that could be folded into a single atomic increment? Tracked in [[open_questions]].
